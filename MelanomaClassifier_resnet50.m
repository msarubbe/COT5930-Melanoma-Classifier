...
%% Resnet 50 Classifier Training 

resnet_50_classifier = trainNetwork(trainingDatastore,lgraph,options);
%% Resnet 50 Clasifier Testing on Unseen Data

YPred = classify(resnet_50_classifier,testingDatastore);

YValidation = testingDatastore.Labels;
accuracy_unseen = mean(YPred == YValidation);
fprintf("The validation accuracy is: %.2f %%\n", accuracy_unseen * 100);
%% Reesnet 50 Classifier Testing on More Unseen Data
[YPred,scores] = classify(resnet_50_classifier,random_unused_test);
...
%Sourced from Appendix 2 of https://www.mathworks.com/help/deeplearning/ug/exp-mgr-weight-init-example.html
