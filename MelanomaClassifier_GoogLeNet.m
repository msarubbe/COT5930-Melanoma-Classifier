...
%% GoogLeNet Classifier Training 

googLeNet_classifier = trainNetwork(trainingDatastore,lgraph,options);
%% GoogLeNet Clasifier Testing on Unseen Data

[YPred,scores] = classify(googLeNet_classifier,testingDatastore);

YValidation = testingDatastore.Labels;
accuracy_unseen = mean(YPred == YValidation);
fprintf("The validation accuracy is: %.2f %%\n", accuracy_unseen * 100);
%% GoogLeNet Classifier Testing on More Unseen Data
[YPred,scores] = classify(googLeNet_classifier,random_unused_test);
